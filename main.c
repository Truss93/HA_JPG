...
int main(int argc, char** argv)
{
	...
	if (quantization_factor < 50)
	{
		factor = 5000.0 / quantization_factor;
	}
	else
	{
		factor = 200.0 - (2.0 * quantization_factor);
	}
	...
	for (int i = 0; i < 64; i++)
	{
		quant_matrix[i] = MAX(8, (int)floor(((factor * default_quant_matrix[i]) + 50.0f) / 100.0f));
	}
